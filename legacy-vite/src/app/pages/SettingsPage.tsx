...
function Toggle({ checked, onChange, disabled = false }: ToggleProps) {
  return (
    <button
      ...
      disabled={disabled}
      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--ring)] focus:ring-offset-1 disabled:opacity-50 disabled:cursor-not-allowed ${
        checked ? "bg-[var(--glin-accent)]" : "bg-[var(--switch-background)]"
      }`}
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs font-medium ${
                deterministicDefault ? "text-[var(--glin-accent)]" : "text-muted-foreground"
              }`}
            >
              deterministic
            </span>
          ...
          <div className="flex flex-wrap gap-1.5">
            {["KR-SBERT", "ko-sroberta", "paraphrase-KoBERT"].map((m) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all font-mono ${
                  embeddingModel === m
                    ? "text-white"
